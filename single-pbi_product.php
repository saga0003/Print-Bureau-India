...
$gallery = function_exists('pbi_product_gallery_images')
    ? pbi_product_gallery_images($product_id)
    : [];
...
$statewide_intro = function_exists('pbi_product_karnataka_copy')
    ? pbi_product_karnataka_copy($product_id)
    : '';
?>
...
<div class="pbi-wrap pbi-r3-shell">
  <div class="pbi-r3-breadcrumbs">
    ...
    <span>›</span>
    <a href="<?php echo esc_url(get_post_type_archive_link('pbi_product')); ?>">Products</a>
    <span>›</span>
  ...
  <div class="pbi-r3-product__hero">
      ...
      <?php if ($gallery_count > 1): ?>
        <div class="pbi-r3-thumbs" role="list" aria-label="<?php the_title_attribute(); ?> gallery">
          <?php foreach ($gallery as $index => $image): ?>
            <button
              class="pbi-r3-thumb<?php echo $index === 0 ? ' is-active' : ''; ?>"
              type="button"
              role="listitem"
              data-gallery-thumb
              ...
              aria-label="View image <?php echo esc_attr((string) ($index + 1)); ?> of <?php echo esc_attr((string) $gallery_count); ?>"
        ...
        <div class="pbi-gallery-lightbox" data-gallery-lightbox hidden aria-hidden="true">
          ...
          <div class="pbi-gallery-lightbox__dialog" role="dialog" aria-modal="true" aria-label="<?php the_title_attribute(); ?> image viewer">
            ...
            <figure>
              ...
              <figcaption>
                <span data-gallery-caption><?php echo esc_html($first_image['alt']); ?></span>
                <span data-gallery-counter>1 / <?php echo esc_html((string) $gallery_count); ?></span>
              </figcaption>
            </figure>
          </div>
        </div>
    ...
    <aside class="pbi-r3-card pbi-r3-quick">
      ...
      <form method="get" action="<?php echo esc_url(home_url('/quote/')); ?>">
        ...
        <div class="pbi-field">
          <label>Size</label>
          <select name="size"><?php foreach ($size_options as $v): ?><option><?php echo esc_html($v); ?></option><?php endforeach; ?></select>
        </div>
        <div class="pbi-field">
          <label><?php echo $is_brochure ? 'Pages' : 'Format'; ?></label>
          <select name="format">
            <option><?php echo $is_brochure ? '8 Pages' : 'Standard'; ?></option>
            <option><?php echo $is_brochure ? '16 Pages' : 'Custom'; ?></option>
          </select>
        </div>
        <div class="pbi-field">
          <label>Paper</label>
          <select name="paper"><?php foreach ($paper_options as $v): ?><option><?php echo esc_html($v); ?></option><?php endforeach; ?></select>
        </div>
        <div class="pbi-field">
          <label>Finish</label>
          <select name="finish"><?php foreach ($finish_options as $v): ?><option><?php echo esc_html($v); ?></option><?php endforeach; ?></select>
        </div>
        <div class="pbi-field">
          <label>Quantity</label>
          <input type="number" name="quantity" min="1" value="250">
        </div>
        <div class="pbi-field">
          <label>Delivery</label>
          <select name="delivery">
            <option><?php echo esc_html($turnaround); ?></option>
  ...
  <div class="pbi-r3-section-title">
    <h2><?php echo $is_brochure ? 'Popular Brochure Styles' : 'Popular Options'; ?></h2>
    <a href="<?php echo esc_url($quote_url); ?>">View all options ↗</a>
  </div>
  ...
  <div class="pbi-r3-style-grid">
    <?php foreach ($style_labels as [$label, $sub, $imgfile]): $img = $asset($imgfile); ?>
      <a class="pbi-r3-style-card" href="<?php echo esc_url($quote_url); ?>">
        <div class="pbi-r3-style-card__image"><?php if ($img): ?><img src="<?php echo esc_url($img); ?>" alt="<?php echo esc_attr($label . ' - ' . get_the_title()); ?>" loading="lazy"><?php endif; ?></div>
  ...
  <?php if (trim(wp_strip_all_tags(get_the_content()))): ?>
    <section class="pbi-product-copy pbi-product-seo-copy">
      <h2><?php the_title(); ?> Printing in Chikmagalur & Karnataka</h2>
      <?php the_content(); ?>
    </section>
  <?php endif; ?>
  ...
  <?php if ($statewide_intro): ?>
    <section class="pbi-product-statewide">
      ...
      <div class="pbi-product-statewide__locations" aria-label="Major Karnataka service locations">
        <span>Chikmagalur / Chikkamagaluru</span>
        <span>Bengaluru</span>
        <span>Mysuru</span>
        <span>Mangaluru</span>
        <span>Hassan</span>
